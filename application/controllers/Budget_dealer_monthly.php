...
class Budget_dealer_monthly extends Root_Controller
{
    ...
    private function system_save()
    {
        ...
        foreach($items as $variety_id=>$packs_detail)
        {
            foreach($packs_detail as $pack_size_id=>$quantity_details)
            {
                ...
                $data_total['quantity_budget_finalize_total']=$data_total['quantity_budget_total'];
    ...
    private function system_forward($id)
    {
        if(isset($this->permissions['action7'])&&($this->permissions['action7']==1))
        {
            ...
            $user_ids[$data['item']['user_finalized']]=$data['item']['user_finalized'];
            ...
            foreach($results as $result)
            {
                $total_crops[$result['crop_id']]['crop_name']=$result['crop_name'];
                if(isset($total_crops[$result['crop_id']]['quantity_budget_total']))
                {
                    $total_crops[$result['crop_id']]['quantity_budget_total']+=$result['quantity_budget_total'];
                }
                else
                {
                    $total_crops[$result['crop_id']]['quantity_budget_total']=$result['quantity_budget_total'];
                }
                if(isset($total_crops[$result['crop_id']]['amount_price_net']))
                {
                    $total_crops[$result['crop_id']]['amount_price_net']+=$result['amount_price_net'];
                }
                else
                {
                    $total_crops[$result['crop_id']]['amount_price_net']=$result['amount_price_net'];
    ...
    private function system_get_variety()
    {
        ...
        foreach($results as $result)
        {
            $item=$this->initialize_row($result['crop_name'],$result['crop_type_name'],$result['variety_name'],$result['variety_id'],$result['pack_size'],$result['pack_size_id'],$result['amount_price_net']);
            if(isset($details_old[$result['variety_id']][$result['pack_size_id']]))
            {
                foreach($details_old[$result['variety_id']][$result['pack_size_id']] as $dealer_id=>$info)
                {
    ...
    private function initialize_row($crop_name,$crop_type_name,$variety_name,$variety_id,$pack_size,$pack_size_id,$amount_price_net)
    {
