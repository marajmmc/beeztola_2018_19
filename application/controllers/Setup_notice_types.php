...
class Setup_notice_types extends Root_Controller
{
    public $message;
    public $permissions;
    ...
    {
        $this->db->from($this->config->item('table_pos_setup_notice_types').' type');
    ...
    private function system_add()
    {
        ...
        {
            $data['title']="Create New Notice Type";
            $data['item']['id']=0;
            $data['item']['name']='';
    ...
    private function system_edit($id)
    {
        if(isset($this->permissions['action2'])&&($this->permissions['action2']==1))
        {
            ...
            $data['item']=Query_helper::get_info($this->config->item('table_pos_setup_notice_types'),array('*'),array('id ='.$item_id),1,0,array('id ASC'));
            if(!$data['item'])
            {
            ...
            $data['title']="Edit Notice Type :: ". $data['item']['name'];
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view($this->controller_url."/add_edit",$data,true));
            if($this->message)
            {
    ...
    private function system_save()
    {
        ...
        if($id>0)
        {
            ...
            Query_helper::update($this->config->item('table_pos_setup_notice_types'),$item, array('id='.$id), false);
        }
        else
        {
            ...
            Query_helper::add($this->config->item('table_pos_setup_notice_types'),$item, false);
        }
