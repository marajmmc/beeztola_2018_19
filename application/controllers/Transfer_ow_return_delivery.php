...
class Transfer_ow_return_delivery extends Root_Controller
{
    public $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Transfer_ow_return_delivery');
        $this->controller_url='transfer_ow_return_delivery';
    }
    ...
    private function system_list()
    {
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            $data['title']="Outlet to HQ Transfer Return Delivery";
            $ajax['status']=true;
    ...
    private function system_get_items()
    {
        ...
        $this->db->where('transfer_ow.status_delivery',$this->config->item('system_status_pending'));
    ...
    private function system_list_all()
    {
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            $data['title']="Outlet to HQ Transfer Return Delivery All List";
            $ajax['status']=true;
    ...
    private function system_edit($id)
    {
        ...
        if(isset($this->permissions['action2'])&&($this->permissions['action2']==1))
        {
            ...
            $data['courier']=Query_helper::get_info($this->config->item('table_sms_transfer_ow_courier_details'),array('*'),array('transfer_ow_id='.$item_id),1);
            if(!$data['courier'])
            {
                $data['courier']['date_delivery']='';
                $data['courier']['date_challan']='';
                $data['courier']['challan_no']=Barcode_helper::get_barcode_transfer_outlet_to_warehouse($data['item']['id']);
    ...
    private function system_save()
    {
        ...
        $result['courier']=Query_helper::get_info($this->config->item('table_sms_transfer_ow_courier_details'),array('*'),array('transfer_ow_id='.$id));
        if($result['courier'])
        {
            if(strtotime($courier['date_challan']))
            {
            ...
            Query_helper::update($this->config->item('table_sms_transfer_ow_courier_details'),$courier, array('transfer_ow_id='.$id));
        }
        else
        {
            if(strtotime($courier['date_challan']))
            ...
            $courier['transfer_ow_id']=$id;
            $courier['date_delivery']=System_helper::get_time($courier['date_delivery']);
            $courier['revision_count']=1;
            $courier['date_updated']=$time;
            $courier['user_updated']=$user->user_id;
            Query_helper::add($this->config->item('table_sms_transfer_ow_courier_details'),$courier);
        }
    ...
    private function system_challan_print($id)
    {
        if(isset($this->permissions['action4'])&&($this->permissions['action4']==1))
        {
            ...
            $data['title']="Outlet to HQ Transfer Return Delivery Print View :: ". Barcode_helper::get_barcode_transfer_outlet_to_warehouse($data['item']['id']);
            $ajax['status']=true;
    ...
    private function system_save_delivery()
    {
        ...
        foreach($result['items'] as $item)
        {
            ...
            $data['out_ow']=($current_stocks[$item['variety_id']][$item['pack_size_id']]['out_ow']+$item['quantity_approve']);
