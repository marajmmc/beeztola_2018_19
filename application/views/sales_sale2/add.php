...
<form id="sale_form" action="<?php echo site_url($CI->controller_url.'/index/save');?>" method="post">
    ...
    <input type="hidden" name="item[discount_self_percentage]" id="discount_self_percentage" value="0">
    ...
    <div class="row widget">
        ...
        <div style="overflow-x: auto;" class="row show-grid" id="container_sale_items">
            <table class="table table-bordered">
                ...
                <tr>
                    <td colspan="10">&nbsp;</td>
                    <td><label><?php echo $CI->lang->line('LABEL_DISCOUNT'); ?>(<label id="discount">0</label>%)</label></td>
...
<div id="system_content_add_more" style="display: none;">
    <table>
        ...
        <tr>
            ...
            <td class="text-right">
                <label class="discount_percentage_variety_label">&nbsp;</label>
                <input class="discount_percentage_variety" type="hidden" value="0"/>
            </td>
            <td class="text-right">
...
<script type="text/javascript">
    ...
    if(sizeof($discounts_slabs)>0)
    {
        ?>
        var discounts_slabs=JSON.parse('<?php echo json_encode($discounts_slabs);?>');
        <?php
    }
    else
    {
    ...
    function calculate_sale_total()
    {
        ...
        $("#container_sale_items tbody .quantity").each( function( index, element )
        {
            ...
            var discount_variety=(price*sale_varieties_info[variety_barcode]['discount_percentage_variety']/100);
        ...
        var discount_customer=0;
        $.each( discounts_slabs, function( index, element )
        {
            if(total_price_actual>= index)
            {
                discount_customer=element;
            }
        });
        $("#discount_self_percentage").val(discount_customer);
        $("#discount").text(discount_customer);

        var total_discount_farmer=(total_price_actual*discount_customer)/100;
    ...
    function add_variety()
    {
        ...
        else
        {
            if(($('#'+'quantity_'+variety_barcode).length)>0)
            {
                ...
                //$('#'+'weight_kg_'+variety_barcode).html(number_format(cur_quantity*sale_varieties_info[variety_barcode]['pack_size']/1000,3,'.',''));
            ...
            else
            {
                ...
                $(content_id+' .discount_percentage_variety_label').html(sale_varieties_info[variety_barcode]['discount_percentage_variety']);
                $(content_id+' .discount_percentage_variety').val(sale_varieties_info[variety_barcode]['discount_percentage_variety']);
