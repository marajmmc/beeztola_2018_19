            ...
            <div style="overflow-x: auto;" class="row show-grid">
                <table class="table table-bordered">
                    ...
                    foreach($items as $index=>$value)
                    {
                        ...
                        ?>
                        <tr id="item_rows_<?php echo $index+1;?>" class='<?php if($quantity_approve!=$quantity_receive){echo 'bg-danger';}?>'>
                            <td>
                            ...
                            <td>
                                <input type="text" value="<?php echo System_helper::get_string_quantity($quantity_receive); ?>" id="quantity_receive_<?php echo $index+1;?>" data-current-id="<?php echo $index+1;?>" name="items[<?php echo $index+1;?>][quantity_receive]" class="form-control integer_type_positive quantity_receive" />
                            </td>
                            <td class="text-right">
                                <label class="" id="quantity_receive_kg_<?php echo $index+1;?>"> <?php echo System_helper::get_string_kg($quantity_receive_kg);?> </label>
                            </td>
                            <td class="text-right">
                                <label class="control-label stock_quantity_new " id="stock_quantity_new_<?php echo $index+1;?>" data-current-id="<?php echo $index+1;?>">
                                    <?php echo System_helper::get_string_quantity($stock_quantity_new); ?>
                                </label>
                            </td>
                            <td class="text-right">
                                <label class="control-label stock_quantity_new_kg " id="stock_quantity_new_kg_<?php echo $index+1;?>" data-current-id="<?php echo $index+1;?>">
                                    <?php echo System_helper::get_string_kg($stock_quantity_new_kg); ?>
                                </label>
                            </td>
                        </tr>
                    <?php
        ...
        <div class="row show-grid">
            ...
            <div class="col-sm-4 col-xs-4">
                <div class="action_button pull-right">
                    <button id="button_action_save" type="button" class="btn" data-form="#save_form" data-message-confirm="Are You Want to Showroom to Showroom Transfer Received?">Save</button>
                </div>
            </div>
            <div class="col-sm-4 col-xs-4">
...
<script>
    $(document).ready(function()
    {
        ...
        $(document).on('input','#items_container .quantity_receive',function()
        {
            ...
            $("#quantity_receive_kg_"+current_id).html(get_string_kg(quantity_receive_kg));
            $("#stock_quantity_new_"+current_id).html(get_string_quantity(stock_quantity_new));
            $("#stock_quantity_new_kg_"+current_id).html(get_string_kg(stock_quantity_new_kg));

            $('#item_rows_'+current_id).removeClass('bg-danger');
            if(quantity_approve!=quantity_receive)
            {
                $('#item_rows_'+current_id).addClass('bg-danger');
            }
            calculate_total();
        });
        ...
        function calculate_total()
        {
            ...
            $('#quantity_total_receive').html(get_string_quantity(quantity_total_receive));
            $('#quantity_total_receive_kg').html(get_string_kg(quantity_total_receive_kg));

            $('#stock_quantity_total_new').html(get_string_quantity(stock_quantity_total_new));
            $('#stock_quantity_total_new_kg').html(get_string_kg(stock_quantity_total_new_kg));
        }
    });
</script>